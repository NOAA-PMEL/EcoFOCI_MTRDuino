...
int ADCRead(int numTries, DateTime timestamp){
  ...
  Wire.requestFrom(ads1100write, 3);  //Take sample from ADC
      ...
      SDCardWrite(adcVal, therm_voltage,numTries,timestamp,b1,b2,controlRegister);
  return adcVal;
}
...
void setAlarmTime(DateTime alarmtime){
    ...
    RTCWrite(0x8E,B00000101);
}


//routine to write a sensor value and a formatted time/date string to SD Card
void SDCardWrite(int val, float voltage, int numTries, DateTime timestamp, byte b1, byte b2, byte controlRegister){
  ...
  if (dataFile) {   // if the file is available, write to it:
    ...
    dataFile.print(" V, ");
    dataFile.print(" creg= ");
    dataFile.print(controlRegister, BIN);
