...
#define RTCPIN 7        //Define Output Pin for RTC 
#define SDPIN 10          //Define Output Pin for SDCARD
#define POWERPIN 8      //Define Digital Output Pin for Power Valve 
...
#define ads1100read 0x49     //Define ADC control register
...
void setup () {    
    ...
    ADCSetup();  //Set gain to 2 on ADC (more precise)
    long adcVal = 0;
    float therm_voltage = 0;
    for (int j = 0;j < numSamples;j++){  //Take a number of samples and everage them to reduce noise
      adcVal += ADCRead();
      delay(100);  //short 10 ms delay before next reading
    }
    //therm_voltage = therm_voltage / numSamples;  //divide accumulated thermistor voltage to get average value
    //float therm_resistance = therm_voltage;  //Calculate value of thermistor resistance based on wheatstone bridge arrangement
    ...
    SDCardWrite(adcVal,therm_voltage,now);
...
void loop () {   
      ...
      float adcTot = 0;
      int adcVal = ADCRead();
      float therm_voltage = 0;  //voltage is adc reading * reference voltage / adc possible values / adc gain
      int refRes = 10000;
      for (int j = 0;j < numSamples;j++){  //Take a number of samples and everage them to reduce noise
        adcVal = ADCRead();
        adcTot+=adcVal;
      ...
      Serial.print(adcTot);
      Serial.print(", ");
      adcTot = adcTot/numSamples;
      Serial.println(adcTot);
      ...
      boolean waiting=false;  //variable to wait for user input
      ...
      while(waiting==true){        //Run loop to set unit status as long as serial connection exists
        ...
        switch (byteRead){   
          ...
          case  83 : //if user enters 'S', display logger "Status"
            ...
            Serial.print("Thermistor Voltage (V):   ");
            Serial.println(therm_voltage, 5);
      ...
      delay(10000);   //wait a few seconds before running the loop again
}

int ADCRead(){
  byte controlRegister =0;
    int adcVal = 0;
      Wire.requestFrom(ads1100read, 3);  //Take sample from ADC
...
void SDCardWrite(int val, float voltage, DateTime timestamp){
  pinMode(10,OUTPUT);    //prevent errors
  SD.begin(SDPIN);  
