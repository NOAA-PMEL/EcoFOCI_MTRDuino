...
void setup () {    
    ...
    //therm_resistance = ??;  //Calculate value of thermistor resistance based on wheatstone bridge arrangement
...
void loop () {   
    ...
    for(int loops=0;loops<1;loops++){
      ADCSetup(1+loops%4);  //Set gain to 2 on ADC (more precise)
      delay(100);
      adcVal=0;
      numTries=0;
      while(adcVal==0){
        for (int j = 0;j < numSamples;j++){  //Take a number of samples and everage them to reduce noise
          adcVal += ADCRead();
          delay(5);  //short 10 ms delay before next reading
        }
        numTries++;
      adcVal = adcVal/numSamples;
      Serial.print("adcVal: ");
      ...
      delay(1000);   //wait a few seconds before running the loop again
      Serial.println();
}
...
int ADCRead(){
  ...
  int adcVal = 0;
      Wire.requestFrom(0x49, 3);  //Take sample from ADC
