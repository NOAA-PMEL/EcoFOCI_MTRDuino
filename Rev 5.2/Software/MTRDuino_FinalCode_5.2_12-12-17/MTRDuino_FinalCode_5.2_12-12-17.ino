...
void setup(){    
  ...
  delay(500);                                                //delay to let voltage levels stabilize
...
void userSetSampleInterval(){
  ...
  if(sampleInterval!=0){
    ...
    if(sampleIntervalFile){                                                 //if the file exists
      ...
      sampleIntervalFile.println(F(" sec"));                                //write 'sec' to file
      Serial.print(F("\n*Sample interval set to "));                        //print confirmation to serial monitor to tell user that the sample interval has been set
...
unsigned long userInputSampleInterval(){
  ...
  Serial.print(F("->"));                                                 //print prompt to note where user input is
  ...
  if(sampleInterval>2419200||sampleInterval==0){                                                                     //if sample interval is too large
      verifyInput=false;                                                                          //flag an error
  }    
  ...
  if(verifyInput==true){                                                                          //if there are no errors in the input 
    ...
    return 0;                                                                                     //return 0 as sample interval
  }
}
...
void userInputDateTime(boolean RTCorFirstSample){    //function to collect date and time from user, boolean variable to tell function if this is for unit's time or first sample
  ...
  Serial.print(F("->"));                             //print prompt to note where user input is
  char inputbuffer[8];                               //buffer for user input
  boolean verifyInput = true;                        //variable to check if user input is actually digits
  ...
  if(verifyInput==true){                               //if no errors were flagged
    ...
    Serial.print(hh);                                //print hours value to serial monitor
    Serial.print(F(":"));                              //print ':' to serial monitor 
    if(mm<10){                                         //if month value is less than 10
      Serial.print(F("0"));                            //display a zero in the first digit place so format looks right
    }
    Serial.print(mm);                                //print minutes value to serial monitor 
    Serial.print(F(":"));                              //print ':' to serial monitor 
    if(ss<10){                                         //if seconds value is less than 10
      Serial.print(F("0"));                            //display a zero in the first digit place so format looks right 
    }
    Serial.println(ss);                              //print seconds value to serial monitor
  }
...
void displayStatus(){
  ...
  Serial.println(F(" --Unplug from USB cable BEFORE FIRST SAMPLE TIME to begin\n"));  //print dialog to explain how to start program
}
